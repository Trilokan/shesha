...
class SaleReturn(models.Model):
    ...
    return_detail = fields.One2many(comodel_name='sale.return.detail',
    ...
    def trigger_grn(self):
        ...
        for rec in recs:
            if (rec.quantity > 0) and (rec.unit_price > 0):
                ...
                line_data = {"reference": self.name,
                             "source_location_id": self.env.user.company_id.location_sale_id.id,
                             "destination_location_id": self.env.user.company_id.location_pharmacy_id.id,
                             "picking_type": "in",
        ...
        if hos_move:
            ...
            data["sale_return_id"] = self.id
            data["source_location_id"] = self.env.user.company_id.location_sale_id.id
            data["destination_location_id"] = self.env.user.company_id.location_pharmacy_id.id
            data["picking_category"] = "material_intake"
    ...
    def trigger_so_approve(self):
        ...
        writter = "Sale Return approved by {0}".format(self.env.user.name)
        self.write({"progress": "approved", "writter": writter})

    @api.multi
    def trigger_cancel(self):
        person_id = self.env["hos.person"].search([("id", "=", self.env.user.person_id.id)])
        writter = "Sale Return cancelled by {0}".format(person_id.name)
